...
namespace ASTImpl
{
    ...
    public:
        ...
        virtual double Evaluate(FunctionPtr & CellCalcFunc) const = 0;
...
namespace
{
    ...
    public:
        ...
        double Evaluate(FunctionPtr & CellCalcFunc) const override
        {
            double lhs = lhs_.get()->Evaluate(CellCalcFunc);
            double rhs = rhs_.get()->Evaluate(CellCalcFunc);
            switch (type_)
            {
            case Add:
                return lhs + rhs;
    ...
    public:
        ...
        double Evaluate(FunctionPtr & CellCalcFunc) const override
        {
            ...
            {
            case UnaryPlus:
                return operand_->Evaluate(CellCalcFunc);
            case UnaryMinus:
                return -operand_->Evaluate(CellCalcFunc);
            }
            return operand_->Evaluate(CellCalcFunc);
...
double FormulaAST::Execute(FunctionPtr & CellCalcFunc) const
{
    return root_expr_->Evaluate(CellCalcFunc);
}

FormulaAST::FormulaAST(std::unique_ptr<ASTImpl::Expr> root_expr, std::forward_list<Position> cell)
: root_expr_(std::move(root_expr)), cells_(std::move(cell))
{
